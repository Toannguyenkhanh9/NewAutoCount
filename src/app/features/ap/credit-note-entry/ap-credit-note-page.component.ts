...
export class ApCreditNotePageComponent {
  ...
  private normalizeAllocationsToTotal(): void {
    ...
    for (let i = this.knockFA.length - 1; i >= 0 && over > 0; i--) {
      ...
      fg.patchValue({ sel: p0 > 0, pay: p0, outstanding }, { emitEvent: false });
  ...
  toggleAlloc(i: number, ev: Event) {
    ...
    if (!checked) {
      ...
      fg.patchValue({ sel: false, pay: 0, outstanding: out }, { emitEvent: false });
  ...
  onPayChanged(i: number) {
    ...
    fg.patchValue({ sel: pay > 0, pay: +pay.toFixed(2), outstanding }, { emitEvent: false });
    this.recalcTotals();
  }
  autoAllocate() {
    ...
    this.knockFA.controls.forEach((c: any) => {
      ...
      c.patchValue({ sel: false, pay: 0, outstanding: +org.toFixed(2) }, { emitEvent: false });
    });
    ...
    this.knockFA.controls.forEach((c: any) => {
      ...
      const outstanding = +(org - pay).toFixed(2);
      c.patchValue({ sel: pay > 0, pay: +pay.toFixed(2), outstanding }, { emitEvent: false });
 ...
 docNo: this.getNextDocNo(),
  ...
  buildOrNoSuggestions() {
    // text đang gõ
    const raw = String(this.rpForm.get('docNo')?.value || '').toUpperCase();
  ...
  private resetAllAllocations(): void {
    this.knockFA.controls.forEach((c) => {
      ...
      const org = +fg.get('orgAmt')!.value || 0;
      fg.patchValue({ sel: false, pay: 0, outstanding: +org.toFixed(2) }, { emitEvent: false });
    });
    this.recalcTotals();
  }
